...
> sleep 不是节奏瓶颈），默认窗口 **120 分钟**（可用工作流参数 `transient_retry_window_minutes` 覆盖），
> 安全上限 120 次。**窗口计量的是"距上一次有进展的尝试"的时间**：单次尝试有效运行 ≥5 分钟后
> 才失败会自动重置窗口（防止长步骤的正常工作时间误吃故障预算，fb4f4e5b7272 实证）。
> 日志为 `[RETRY] 上游/网络瞬时故障，Xs 后重试（时间盒 N/120 分钟）`。
...
| **上游路由/可用性故障** | 转发站智能路由的上游不可用或超时，与本地请求内容无关 | HTTP 504（Cloudflare 错误页）/ `upstream_unavailable` / DNS `getaddrinfo`；按转发站建议切换固定商家或稍后重试 |
| **请求体字段被严格上游拒绝** | 请求携带了上游不认识的字段（如响应项 `status` 被回放进 input） | 400 响应体含 `"param":"input[N].xxx"` 字段路径；已于 2026-09-03 修复 status 回放（`openai_responses_agent._continuation_items` 剥离 status）。注意：与上游路由故障是**两类独立问题**，不要互相归因 |
