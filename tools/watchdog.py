#!/usr/bin/env python3
"""
watchdog.py — Server-side unified monitoring daemon for Vibe Research.
...
Usage:
    ...
    python3 watchdog.py [--base-dir /tmp/Vibe Research-watchdog] [--interval 60]
...
DEFAULT_BASE = "/tmp/Vibe Research-watchdog"
DEFAULT_INTERVAL = 60
...
def check_download(task, status_dir, interval):
    ...
    if 0 < speed < SLOW_SPEED_THRESHOLD:
        return write_status(status_file, {
            ...
            "size": current_size, "speed_mbps": round(speed / 1024 / 1024, 2), "ts": now,
        })
    return write_status(status_file, {
        "status": "OK", "task": name, "type": "download",
        "size": current_size, "speed_mbps": round(speed / 1024 / 1024, 2), "ts": now,
...
def main():
    parser = argparse.ArgumentParser(description="Vibe Research Watchdog — server-side task monitoring daemon")
