...
Given a broad research direction from the user, systematically generate, validate, and rank concrete research ideas. This skill composes with the research-lit, novelty-check, and research-review steps to form a complete idea discovery pipeline.
...
0. **Read previous step outputs**: Check if `literature_review.md` or `references.bib` exist in the project root (produced by the `research-lit` step). If found, read them first to build upon the existing literature survey rather than duplicating the work.
