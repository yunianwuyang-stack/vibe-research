...
- **EXA_FETCHER** — canonical name `exa_search.py`, resolved per
  ...
  (Codex-side chain: `$VIBE_REPO/tools/` → `tools/` → `~/.codex/skills/exa-search/`).
...
EXA_FETCHER=""
[ -n "${VIBE_REPO:-}" ] && [ -f "$VIBE_REPO/tools/exa_search.py" ] && EXA_FETCHER="$VIBE_REPO/tools/exa_search.py"
...
[ -z "$EXA_FETCHER" ] && {
  echo "ERROR: exa_search.py not resolved at \$VIBE_REPO/tools/, tools/, or ~/.codex/skills/exa-search/." >&2
  echo "       Fix: rerun bash tools/install_vibe_codex.sh, export VIBE_REPO, or copy the helper to tools/." >&2
...
```
exa_search.py not found. Run install_vibe_codex.sh, set VIBE_REPO to your Vibe Research repo root, or install/copy the helper into the project/global Codex skill path; then install exa-py:
pip install exa-py
```
