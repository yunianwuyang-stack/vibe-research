...
- [ ] TikZ architecture diagram: check against plan document if planned diagrams were generated
...
# Check TikZ architecture diagram (against plan)
...
if [ "$tikz_exists" = "NO" ]; then
    for plan in PAPER_PLAN.md PROBLEM_ANALYSIS.md TOPIC_PLAN.md; do
        if [ -f "$plan" ] && grep -qi 'tikz\|architecture.*diagram\|roadmap\|framework.*diagram\|flow.*chart' "$plan" 2>/dev/null; then
            echo "⚠️ Plan includes TikZ diagrams but figures/tikz_architecture_examples.tex not found"
            break
        fi
    done
fi
