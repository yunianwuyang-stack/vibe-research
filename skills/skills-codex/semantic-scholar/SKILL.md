...
- **S2_FETCHER** — canonical name `semantic_scholar_fetch.py`, resolved per
  ...
  (Codex-side chain: `$VIBE_REPO/tools/` → `tools/` → `~/.codex/skills/semantic-scholar/`).
...
S2_FETCHER=""
[ -n "${VIBE_REPO:-}" ] && [ -f "$VIBE_REPO/tools/semantic_scholar_fetch.py" ] && S2_FETCHER="$VIBE_REPO/tools/semantic_scholar_fetch.py"
