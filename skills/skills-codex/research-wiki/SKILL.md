...
The research wiki is a persistent, per-project knowledge base that accumulates structured knowledge across the entire Vibe Research research lifecycle. Unlike one-off literature surveys that are used and forgotten, the wiki **compounds** — every paper read, idea tested, experiment run, and review received makes the wiki smarter.
...
canonical helper `python3 "$VIBE_REPO/tools/research_wiki.py" ingest_paper …`, which
is the single implementation of paper ingest in Vibe Research (per
...
- single canonical helper — `python3 "$VIBE_REPO/tools/research_wiki.py" ingest_paper …`
