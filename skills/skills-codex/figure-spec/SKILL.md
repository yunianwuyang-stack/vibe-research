...
to match the new layout (re-run `install_vibe_codex.sh` to pick up
...
[ -n "${VIBE_REPO:-}" ] && [ -f "$VIBE_REPO/skills/figure-spec/scripts/figure_renderer.py" ] && FIGURE_RENDERER="$VIBE_REPO/skills/figure-spec/scripts/figure_renderer.py"

# Layers 1-3: legacy shared-runtime chain via shim at tools/figure_renderer.py.
[ -z "$FIGURE_RENDERER" ] && [ -n "${VIBE_REPO:-}" ] && [ -f "$VIBE_REPO/tools/figure_renderer.py" ] && FIGURE_RENDERER="$VIBE_REPO/tools/figure_renderer.py"
[ -z "$FIGURE_RENDERER" ] && [ -f tools/figure_renderer.py ] && FIGURE_RENDERER="tools/figure_renderer.py"

# Layer 4: Codex-side skill-local install (`install_vibe_codex.sh` may place it here).
...
[ -n "$FIGURE_RENDERER" ] || {
  echo "ERROR: figure_renderer.py not found at any of: \$VIBE_REPO/skills/figure-spec/scripts/, \$VIBE_REPO/tools/, tools/, ~/.codex/skills/figure-spec/scripts/, ~/.codex/skills/figure-spec/. Set VIBE_REPO, rerun install_vibe_codex.sh, or copy the canonical \$VIBE_REPO/skills/figure-spec/scripts/figure_renderer.py next to this skill." >&2
  exit 1
}
...
Iterate until all three axes ≥ 7/10. The Vibe Research tech report figures went through 5 rounds of this loop to reach C:7/R:7/S:8.
