...
- **DEEPXIV_FETCHER** — canonical name `deepxiv_fetch.py`, resolved per
  ...
  (Codex-side chain: `$VIBE_REPO/tools/` → `tools/` → `~/.codex/skills/deepxiv/`).
...
DEEPXIV_FETCHER=""
[ -n "${VIBE_REPO:-}" ] && [ -f "$VIBE_REPO/tools/deepxiv_fetch.py" ] && DEEPXIV_FETCHER="$VIBE_REPO/tools/deepxiv_fetch.py"
...
- Use DeepXiv as an additive source, not a replacement for existing Vibe Research literature tooling.
