...
if research-wiki/ exists:
    ...
    for each claim resolved by this verdict:
        if verdict == "yes":
            ...
            run the installed Vibe Research research_wiki.py helper to add a supports edge from "exp:<id>" to "claim:<cid>"
        elif verdict == "partial":
            Update claim page: status → partial
            run the installed Vibe Research research_wiki.py helper to add a partial supports edge from "exp:<id>" to "claim:<cid>"
        else:
            Update claim page: status → invalidated
            run the installed Vibe Research research_wiki.py helper to add an invalidates edge from "exp:<id>" to "claim:<cid>"
    ...
    # 4. Rebuild + log
    rebuild the query pack with the installed Vibe Research research_wiki.py helper
    log "result-to-claim: exp:<id> verdict=<verdict> for idea:<idea_id>" with the installed Vibe Research research_wiki.py helper
