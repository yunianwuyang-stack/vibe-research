...
# or the VIBE_CONDA_HOOK environment variable.
...
- **Natural language description** (Codex parses into manifest)
...
[ -n "${VIBE_REPO:-}" ] || { echo "ERROR: VIBE_REPO not set. Use install_vibe_codex.sh managed install or export VIBE_REPO=/path/to/Vibe Research."; exit 1; }
# Prefer the new canonical location; fall back to legacy tools/ shim path.
QUEUE_TOOLS="$VIBE_REPO/skills/experiment-queue/scripts"
[ -f "$QUEUE_TOOLS/queue_manager.py" ] || QUEUE_TOOLS="$VIBE_REPO/tools/experiment_queue"
[ -f "$QUEUE_TOOLS/queue_manager.py" ] || { echo "ERROR: queue_manager.py not found at $VIBE_REPO/skills/experiment-queue/scripts/ or $VIBE_REPO/tools/experiment_queue/"; exit 1; }
```
...
```bash
REMOTE_RUN_REL=".vibe_queue/runs/$RUN_TS"
...
```bash
ssh <server> "mkdir -p \"$REMOTE_RUN_DIR/logs\" \"\$HOME/.vibe_queue\""
scp "$QUEUE_TOOLS/queue_manager.py" "$QUEUE_TOOLS/build_manifest.py" <server>:.vibe_queue/
...
```bash
ssh <server> "nohup python3 \"\$HOME/.vibe_queue/queue_manager.py\" \\
