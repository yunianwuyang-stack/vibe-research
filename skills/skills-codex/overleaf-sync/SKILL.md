---
name: overleaf-sync
description: "Two-way sync between a local paper directory and an Overleaf project via the Overleaf Git bridge (Premium feature). Lets you keep Vibe Research audit/edit workflows on the local copy while collaborators edit in the Overleaf web UI. Token never touches the agent — user does the one-time auth via macOS Keychain. Use when user says \"同步 overleaf\", \"overleaf sync\", \"推送到 overleaf\", \"connect overleaf\", \"Overleaf 桥接\", \"pull overleaf\", \"push overleaf\", or wants to bridge their Vibe Research paper directory with an Overleaf project."
...
- **Vibe Research** can read your changes, run audits (`/paper-claim-audit`, `/citation-audit`, `/auto-paper-improvement-loop`), and push fixes back
...
- You want to use Overleaf as the editing surface (better collaboration, shared with team) but still run Vibe Research pipelines locally
- You want to take an existing local Vibe Research paper and push it to Overleaf for a co-author to edit
...
│  (Vibe Research audits)  │                          │ (git bridge)    │     (collaborators)
└─────────────────┘                          └─────────────────┘
```

The `paper-overleaf/` directory is a **git clone of the Overleaf project**. The `paper/` directory is the working copy where Vibe Research skills run. They are kept in sync via `rsync`.
...
Run this in your own terminal (NOT through me):
    ...
    bash <VIBE_REPO>/tools/overleaf_setup.sh <project-id-or-url>
...
bash <VIBE_REPO>/tools/overleaf_audit.sh .   # must report "Audit clean"
```
...
Use after Vibe Research skills have edited `paper/` and you want collaborators on Overleaf to see the changes.
...
# 4. Commit + push
git add -A
git commit -m "<descriptive message — what Vibe Research changed and why>"
git push
```

**Commit message protocol**: include the Vibe Research skill that produced the change so collaborators on Overleaf understand provenance. Examples:
...
| No  | Yes | Local Vibe Research edits unsynced | Run `push` |
...
3. Show the user `git log origin/master ^HEAD` (their Overleaf commits) and `git log HEAD ^origin/master` (local Vibe Research commits).
...
- If the user is in an active Overleaf editing session, Vibe Research skills should **read-only** access `paper/` until the user runs `/overleaf-sync pull`.
- If Vibe Research is in the middle of `/auto-paper-improvement-loop` or `/paper-write`, the user should pause Overleaf editing until the loop finishes and `/overleaf-sync push` is run.
...
- `paper/` directory unchanged in role — still the Vibe Research working copy
