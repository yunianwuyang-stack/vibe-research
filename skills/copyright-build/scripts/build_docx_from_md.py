...
# The bundled embedded Python uses an isolated ``._pth`` and does not add the
# executed script's directory to ``sys.path``.  Make the sibling helper
# import deterministic for both source-tree and packaged execution.
...
def add_inline_runs(paragraph: Any, text: str, size_pt: float = 10.5) -> None:
    """把一段文字按 **粗体** / `代码` 拆成多个 run 写入段落，其余按正文宋体。"""
    pos = 0
...
def add_image(
    ...
    if not image_path.exists():
        ...
        return
    # 尺寸规范化：正常图统一目标宽 14cm；低像素图防糊；瘦长图防溢出
    target_width_cm = 14.0
    max_height_cm = 20.0
...
def build_manual_docx_python(md_path: Path, out_path: Path, base_dir: Path, software_name: str, version: str) -> None:
    ...
    for line in lines:
        ...
        if heading:
            ...
            p = document.add_heading(strip_inline_markers(heading.group(2)), level=level)
            for run in p.runs:
                try:
                    run.font.color.rgb = RGBColor(0, 0, 0)
                except Exception:
        ...
        if re.match(r"^\d+\.\s+", stripped):
            ...
            add_inline_runs(p, re.sub(r"^\d+\.\s+", "", stripped), 10.5)
            continue
        p = document.add_paragraph()
